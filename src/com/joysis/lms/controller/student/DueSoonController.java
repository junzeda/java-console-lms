...
public class DueSoonController {
	...
	public DueSoonController(BorrowBookDAO borrowDAO, BookDAO bookDAO, AuthorDAO authorDAO, CategoryDAO categoryDAO) {
	...
	public void incomingDueBook() {
		Student student = homepageView.getStudent();
		List<BorrowBook> borrows = borrowDAO.getStudentIncomingDue(student.getStudentId());
		if(borrows.isEmpty()) {
			System.out.println("No current transaction.");
		}
		
		System.out.printf("%-119s |\n","	"+ConsoleStyles.colorize("NOTE: Please be reminded that books must be returned on or before the due date.", ConsoleStyles.YELLOW));
		System.out.printf("%-119s |\n","	"+ConsoleStyles.colorize("A fine of ₱10.00 per day will be charged for each day the book is overdue.", ConsoleStyles.YELLOW));
		System.out.printf("%-119s |\n","	"+ConsoleStyles.colorize("To avoid penalties, please check your due dates regularly under 'My Borrowed Books' or 'Due Soon'.", ConsoleStyles.YELLOW));
		...
		for(BorrowBook borrow : borrows) {
			...
			System.out.printf("%-124s |\n",ConsoleStyles.colorize("		Due Date: " + borrow.getDueDate() + " Remaining Day(s): " + remainingDays, ConsoleStyles.RED, ConsoleStyles.BOLD));
